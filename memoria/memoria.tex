...
Para realizar la comparación de los tiempos de ejecución de las funciones se han utilizado las funciones implementadas en el archivo \texttt{profiling.py}.\\

Se ha realizado un análisis de los tiempos de ejecución de las dos implementaciones de la entropía y de las dos implementaciones de la información mutua vistas. Para ello, se han ejecutado 5 veces y nos hemos quedado con los tiempos medios de ejecución para diferentes valores del número de muestras y dimensión de las mismas.\\

Se han considerado tres rangos para el tamaño de las muestras: pequeñas (1000 muestras), medianas (30000 muestras) y grandes (100000). De forma similar, en función a la dimensión, se realizaron estimaciones con tres tipos de dimensiones: pequeñas (dimensión 2), medianas (dimensión 10) y grandes (dimensión 100). Notamos que en el caso de la dimensión 100 el programa no llega a finalizar, así que debemos excluirlo de la tabla de resultados.\\

Comenzaremos analizando los tiempos obtenidos al calcular la entropía. Podemos observarlos en la Tabla \ref{tab:tiempos_ent}. Como es de esperar, a medida que aumenta el número de muestras y la dimensión, se incrementan los tiempos de ejecución. Para dimensión 2, los tiempos son similares para muestras pequeñas y medianas, diferenciándose más para muestras grandes. Sin embargo, en todos los casos la implementación 1 es más lenta que la 2. Al aumentar la dimensión, se incrementan los tiempos de ejecución de ambas implementaciones, pero también las diferencias entre ellas. Para una muestra pequeña la implementación 1 tarda aproximadamente el doble que la 2. Con muestras más grandes la diferencia de tiempo aumenta. Para una muestra grande, la implementación 1 tarda casi el triple que la 2.\\

Podemos concluir, por tanto, que la segunda implementación es más rápida que la primera para calcular la entropía. El cálculo que realizan ambas estimaciones es el mismo, utilizan el mismo estimador, varía únicamente la implementación, en cuanto a tiempo de cómputo será preferible la implementación 2.

---¿qué tabla nos permite comparar mejor? ---

\begin{table}[!htb]
  \begin{adjustwidth}{-.5in}{-.5in}  
    \caption{Tiempos de ejecución de la función \texttt{entropy}.}
...
2 & 10 &  0.0504 $s$ &  32.9856 $s$ &  783.608 $s$\\
\bottomrule
\end{tabular}
\end{table}

En la Tabla \ref{tab:tiempos_mi} vemos los tiempos de ejecución para ambas implementaciones de la información mutua con los tamaño y dimensión de muestra establecidos. Al igual que para la entropía, los tiempos en todos los casos aumentan al considerar muestras más grandes y de mayor dimensión. Además, notamos que los tiempos de ejecución superiores que los necesitados para calcular la entropía, esto no es de extrañar, en uno de los casos se calcula la entropía de varias variables y luego se opera con ellas para obtener la información mutua. Para un tamaño de muestra pequeña y dimensión pequeña, los tiempos son muy similares, pero a medida que van aumentando, tanto dimensión como tamaño de muestra, aumenta también la diferencia entre los tiempos de ejecución de las dos implementaciones. En este caso ocurre al contrario que al calcular la entropía, la implementación que tarda más en estimar la información mutua es, en todos los casos, la segunda, tardando el doble para muestras grandes con dimensión 10.
