...
\begin{theorem}[Desigualdad de Jensen]\label{th:jensen}
  Si $f$ es una función cóncava y $X$ es una variable aleatoria, entonces\[
\E\left[f(X)\right]\ge f\left ( \E[X] \right),
  \]
dándose la igualdad si, y solo si, $X$ es constante o $f$ es lineal casi seguramente, existe un conjunto Borel en el que es lineal.
\end{theorem}
...
\end{theorem}
\begin{proof}
Sea $S$ el soporte de $f$, entonces 
\begin{align*}
  - D\left ( f || g \right ) &= \int_S \log \left(\frac{f(x)}{g(x)}\right ) f(x) dx  \leq \log \left( \int_S f(x)\cdot \frac{g(x)}{f(x)}dx \right)\\
  &= \log \left ( \int_Sg(x)dx\right) \leq 1 = 0.
\end{align*}
La primera desiguadad es la desigualdad de Jensen, Teorema \ref{th:jensen} aplicada al $\log$ que es una función cóncava, la segunda desigualdad será igualdad si, y solo si, $f = g$ en casi todo punto.
\end{proof}
...
\begin{align*}
P_k(\varepsilon_x, \varepsilon_y) &= P_k^{a)}(\varepsilon_x, \varepsilon_y) + P_k^{b)}(\varepsilon_x, \varepsilon_y)\\ &= P_k\left (\epsilon_x, \epsilon_y | A \right) \cdot P (A) + P_k\left (\epsilon_x, \epsilon_y | B \right) \cdot P(B).\\  
\end{align*}
...
\begin{align*}
  I^{(2)}(X,Y) \approx& \ \widehat{h}(X) + \widehat{h}(Y) - \widehat{h}(X, Y)\\ =& - \frac{1}{N}\sum_{i=1}^N\psi(n_x(i)+1) + \psi(N) + \log V_{d_X} + \frac{d_X}{N} \sum_{i=1}^N\log \varepsilon(i)\\
  & - \frac{1}{N}\sum_{i=1}^N\psi(n_y(i)+1)  + \psi(N) + \log V_{d_Y} + \frac{d_Y}{N} \sum_{i=1}^N\log \varepsilon(i)\\
...
En la implementación 2, en primer lugar, se crea un árbol, que será de tipo \texttt{sklearn.neighbors.BallTree} cuando la dimensión sea mayor que 20 y de tipo \texttt{sklearn.neighbors.KDTree} cuando la dimensión sea mejor que 20; y a continuación, se calculan los vecinos más cercanos llamando directamente al método \texttt{query} de la clase correspondiente. La métrica utilizada es la de Chebyshev y \texttt{leaf\_size = 40}.\\

Suponemos que el coste de realizar la llamada en paralelo sin aprovechar realmente el paralelismo es el que provoca el incremento en las llamadas a \texttt{query} en la primera implementación y que por eso en la segunda implementación, en la que se llama al método directamente, es más rápida.---Esto me parece raro pq entiendo que si sklearn usa paralelismo a nivel de hebras es pq GIL estará desactivado---\\

Esta diferencia también podría venir causada por el valor de \texttt{leaf\_size}, la diferencia entre los valores es 10, que comparado con el número de muestras puede parecer pequeño, pero al afectar al cálculo de los vecinos más cercanos de todos los puntos podría afectar notablemente.\\
