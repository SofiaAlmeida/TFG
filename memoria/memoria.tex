...
Es en el ámbito de la teoría de la información donde surgen las definiciones estudiadas en este trabajo. La teoría de la información busca límites teóricos a la capacidad de intercambiar mensajes, así como desarrollar esquemas que alcancen una capacidad óptima. La teoría de la información se puede ver como una rama de la teoría de la probabilidad, aunque también como una rama de la teoría de los sistemas dinámicos.\\

Aunque ya existían conceptos de información, es con los trabajos de Shannon en la década de los 40 cuando realmente se les aporta significado a estos conceptos. Shannon define dos medidas de información, que serán las definiciones centrales de este trabajo. Por un lado, define la \textit{entropía}, a partir de la definición que Hartley había dado en el campo de la termodinámica, como una medida de la ``cantidad de información'' que una variable aleatoria aporta sobre sí misma. Formalmente, para una variable aleatoria discreta $X$ con función masa de probabilidad $p(x)$ se define la entropía, $H(X)$, como\[
H(X) = -\sum_{x\in \mathcal{X}}p(x)\log p(x).
\] Por otro lado, define la \textit{información mutua}, una medida de la cantidad de información que una variable aleatoria nos da sobre otra. Analíticamente, para dos variables aleatorias $X$ e $Y$ discretas con función masa de probabilidad conjunta $p(x,y)$ y funciones masa de probabilidad marginales $p(x), p(y)$ respectivamente, se define la información mutua $I(X;Y)$, como\[
I(X;Y) = \sum_{x\in \mathcal{X}}\sum_{y \in \mathcal{Y}}p(x,y)\log\frac{p(x,y)}{p(x)p(y)}.
\]

Estas medidas se pueden definir también en el caso continuo y no son las únicas medidas de información, tenemos también la entropía relativa, propuesta por Kullback, por ejemplo.\\

Desde un punto de vista estadístico, podemos ver estas medidas de información como una medida de dependencia entre dos variables. De hecho, si comparamos la información mutua con el coeficiente de correlación de Pearson, por ejemplo, veremos que la primera nos da más información que la segunda, ya que no mide un tipo concreto de dependencia y caracteriza la independencia.\\
...
Calcular la entropía o la información mutua resulta de interés no solo en el ámbito de la teoría de la información o en la termodinámica, ya que se les ha encontrado utilidad en numerosos campos como la economía, donde se puede utilizar para estudiar el crecimiento económico, o la biología, para predecir la estructura secundaria del ARN. En el campo del aprendizaje automático se utilizan estimaciones de la entropía para medir cómo de bueno es un modelo o para estimar sus hiperparámetros. La información mutua se puede utilizar como criterio para seleccionar variables, hacer un análisis clúster o detectar causalidades.\\

En este trabajo se estudiarán los conceptos de entropía e información mutua, algunos conceptos relacionados, como la entropía condicionada, y sus propiedades más importantes, además de las relaciones entre los diferentes conceptos. Puesto que estos conceptos dependen de la función masa de probabilidad o función de densidad de las variables aleatorias, que no siempre resulta conocida, se continuará estudiando cómo estimarlas. Para ello, se introducirán brevemente los estimadores basados en particiones y se desarrollarán estimadores basados en los $k$ vecinos más cercanos y de tipo de Kozachenko - Leonenko. Estos estimadores nos permitirán, para una observación de una variable de la que no conocemos su distribución teórica, aproximar la entropía y la información mutua. Por último, se analizarán dos implementaciones de algunos de los estimadores estudiados teóricamente y se realizará una análisis comparativo entre ellas. Este análisis contemplará la calidad de la solución a través de la observación del error y el rendimiento en términos de tiempo de ejecución mediante \textit{profiling}.\\

El trabajo se organiza en una serie de capítulos donde se agrupan los contenidos de la forma explicada a continuación.\\

En el capítulo 2 se comienza repasando los conceptos básicos necesarios en el desarrollo posterior. Entre otras, se comenta la definición de estimador y sus propiedades, algunas distribuciones que aparecerán a lo largo del desarrollo y se definen otras dos medidas de la dependencia como motivación para la definición de la información mutua. Se continúa presentando los conceptos de teoría de la información a estudiar: entropía e información mutua, así como una serie de propiedades y relaciones entre ellas. Este estudio se realizará tanto para el caso discreto como para el continuo. Se sigue estudiando estimadores de los conceptos anteriores. Se verá brevemente un estimador basado en particiones y en más profundidad un estimador de la entropía basado en los $k$ vecinos más cercanos, así como del tipo de Kozachenko - Leonenko, también veremos estimadores de la información mutua de este tipo. Para cerrar este capítulo se analizarán dos implementaciones de los estimadores estudiados anteriormente.\\

En el capítulo 3 se explica la metodología utilizada para la comparación de estimadores. Se comienza comentando brevemente el lenguaje de programación a utilizar y las bibliotecas utilizadas, centrándonos posteriormente en aquellas utilizadas para medir los tiempos de ejecución. Durante el estudio de las implementaciones adquiere interés conocer cómo funciona el paralelismo en Python y las herramientas que proporciona la biblioteca \texttt{scikit-learn} para calcular los vecinos más cercanos, se incluye una explicación de ambas cosas en este punto. Por último, se explican los fundamentos teóricos del $T-Test$ que será posteriormente utilizado para comparar los diferentes estimadores.\\

En el capítulo 4 es donde se encuentran los resultados de los diferentes análisis realizados. En primer lugar se compara el comportamiento de los estimadores con el de una variable aleatoria con distribución normal, de la que conocemos teóricamente su entropía e información mutua. Se continúa realizando un \textit{T-Test} para saber si las diferencias en media de los estimadores son o no significativas. Se finaliza comparando los tiempos de ejecución de ambas implementaciones mediante \textit{profiling}.\\
