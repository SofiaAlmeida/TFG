...
Si todas las densidades son funciones propias (todos sus valores son distintos de $-\infty$ y al menos un valor es menor que $+\infty$), cuando $N$ tienda a infinito y el tamaño de cada partición converja a cero, la estimación $I_{\textit{binned}}(X,Y) \to I(X;Y)$.---Referenciar---¿Teorema 1 - Estimation of Entropy and Mutual Information - Liam Paninski?---
...
Este estimador es sesgado, aunque es asintóticamente insesgado y con $\Var\left (\hat{h}\right)\to \infty$ solo si $k\to \infty, N \to \infty$ y $\frac{k}{N} \to 0$. Aumentar el valor de $k$ incrementa el tiempo de cómputo notablemente, dificultando el cálculo efectivo. Por ello, se propone la modificación siguiente, basada en el estimador de Kozachenko - Leonenko. ---En el artículo de Fernando Pérez Cruz viene probado lo primero antes de aplicar logaritmo, ¿si tomamos logaritmos se sigue cumpliendo?---

\subsection{Estimadores de la entropía del tipo de Kozachenko - Leonenko}

Partiendo de un razonamiento similar al anterior, trataremos de estimar la entropía, $h(X)$, a partir de \[
\widehat{h}(X) = -\frac{1}{N}\sum_{i=1}^N\widehat{\log f}(X_i).\]

La diferencia vendrá marcada por la forma de estimar $\widehat{\log f}(X_i)$.\\
...
Vamos a calcular $P\left[\epsilon \leq \mathcal{E}_{i_k} \leq \epsilon + h \right]$. Esta probabilidad es igual a la probabilidad de que haya un punto a distancia $r\in \left[ \varepsilon,\varepsilon + h \right ]$ de $X_i$, de que haya otros $k-1$ puntos a menor distancia; y de que $N-k-1$ puntos se encuentren a mayor distancia de $x_i$.\\

Llamamos $p_i(\epsilon) = P\left[X_i \in B(x_i,\epsilon)\right]$ a la distribución de la bola centrada en $x_i$ y radio $\varepsilon$, \[
p_i(\varepsilon) = \int_{B(x_i,\epsilon)}f(y)dy.\]

Usaremos la fórmula de la distribución multinomial para expresar $P_k(\varepsilon)$, teniendo en cuenta que, para una muestra aleatoria simple, $p_i(\epsilon) = p_j(\epsilon)$ por ser una muestra idénticamente distribuida.
...
Se comprueba que $\int P_k(\varepsilon)d\varepsilon = 1$, que debe cumplir para ser función de densidad. ---Referenciar, no lo encuentro en el último artículo---

Podemos calcular la esperanza de $\log p_i (\varepsilon)$, a partir de la posición de los $N-1$ puntos restantes, con $x_i$ fijo:
---Referenciar, no lo esncuentro en el último artículo---
\begin{align*}
...
Hemos calculado un estimador de $\widehat{\log f} (X_i)$, sustituiremos de la forma propuesta para obtener un estimador de la entropía:
\begin{align}
...
Por cómo lo hemos obtenido, $\widehat{h}(X)$ será insesgado si la densidad $f(x)$ es constante.
