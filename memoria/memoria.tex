...
\section{Objetivos}
---objetivos, planificación (también se podría poner en objetivos) -> hacer un diagrama de Gantt (revisar bibliografía, adquirir conocimmiento, escribir memoria, búsqueda de implementaciones, escribir código)---

%--------------------------- FUNDAMENTOS--------------------------------
\chapter{Funtamentos y estado del arte}
...
Las definiciones a continuación incluyen integrales, por lo que entenderemos en la redacción que todas ellas existen para no alargar la redacción.\\
...
\begin{example}\label{ej:norm_multi}
  Tomamos una variable aleatoria multidimensional $X = (X_1,\cdots,X_n)$ con distribución normal multivariante de vector de medias $\mu$ y matriz de covarianzas $\Sigma$. Su función de densidad es \[f(x) = \frac{1}{\left (\sqrt{2\pi}\right )^n |\Sigma|^{\frac{1}{2}}} e^{-\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)}.\]
...
\begin{theorem}\label{t:mmlog}
  Sean $X_1,\cdots, X_n$ una serie de variables aleatorias independientes e idénticamente distribuidas con función de densidad $f_X(x)$. Si notamos por $f(x_1,\dots,x_n)$ a la función de densidad conjunta, entonces\[
...
\begin{lemma}\label{l:ent_im}
  Siguiedo la notación anterior, la información mutua de dos variables aleatorias $X$, $Y$ verifica:\[
  I(X;Y) = h(X) + h(Y) - h(X,Y).
  \]
\end{lemma}
...
\begin{proof}
  ...
  Como en el Lema anterior, partiremos de la definición de información mutua y utlizaremos propiedades del logaritmo, el teorema de Fubini y la definición de función de densidad condicionada.
  \begin{align*}
    I(X;Y)&= \int \int  \log\left( \frac{f(x,y)}{f(x)f(y)}\right)f(x,y)dxdy \\
...
Generalmente no conocemos las funciones de densidad de las variables aleatorias cuya entropía o información mutua queremos calcular, por ello, tratamos de aproximar este valor a partir de información que sí conocemos, como realizaciones muestrales de las variables. El objetivo es, para dos variables aleatorias $X,Y$ ($d$-dimensionales), estimar $h(X)$ o $I(X;Y)$ a partir de un conjunto de realizaciones muestrales, $\{z_i=(x_i,y_i) : i = 1,\cdots, N\}$, sin conocer las funciones de densidad $f_z, f_x$ y $f_y$, donde $Z = (X,Y)$ es la distribución conjunta de $X$ e $Y$. Asumiremos en el desarrollo a continuación que todas las funciones de densidad son propias, es decir, que no son idénticamente $-\infty$.\\
...
Podemos ver \[
\frac{1}{N} \sum_{j=1}^NI\left(X_j\in B(x_i,\epsilon_k(x_i))\right),\]donde $I$ es la función indicadora, como un estimador de $P[X_j\in B(x_i,\epsilon_k(x_i))]$. Por la definición de $\epsilon_k(x_i)$ tenemos que \begin{align}\label{eq:knn-ind}
...
P[X_j\in B(x_i,\epsilon_k(x_i))] &\approx \int_{B(x_i,\epsilon_k(x_i))}f(y)dy\approx f(x_i) \int_{B(x_i,\epsilon_k(x_i))}dy\\ &= f(x_i)\cdot V_d \cdot \epsilon^d_k(x_i),
\end{align*}
...
Utilizando \ref{eq:knn-ind} para estimar $P[X_j\in B(x_i,\epsilon_k(x_i))]$, tendremos,\[
...
Consideramos $X_1,\dots, X_n$ una muestra aleatoria simple de la variable aleatoria $X$. Partiendo del mismo razonamiento, trataremos de estimar la entropía, $h(X)$, a partir de \[
...
\begin{align*}
  P_k(\epsilon) &= \lim_{h\to 0}\frac{P\left[\epsilon \leq \mathcal{E}_{i_k} \leq \epsilon + h\right]}{h}\\[5pt] &= \lim_{h\to 0} \left [\frac{(N-1)!}{1!(k-1)!(N-k-1)!}  \cdot p_i(\epsilon)^{k-1} \cdot \left(1-p_i(\epsilon)\right)^{N-k-1}\left(\frac{p_i(\epsilon + h ) - p_i(\epsilon)}{h}\right)\right ]\\[10pt] &= k\binom{N-1}{k}
\cdot \frac{d p_i(\varepsilon)}{d\varepsilon} \cdot p_i(\epsilon)^{k-1} \cdot \left(1-p_i(\epsilon)\right)^{N-k-1}.\\
\end{align*}
...
\begin{align*}
\mathbb{E} \left[\log p_i(\epsilon)\right ] &= \int_0^\infty P_k(\varepsilon) \log p_i(\varepsilon) d\varepsilon =  k\binom{N-1}{k} \int_0^1 p^{k-1}(1-p)^{N-k-1}\log p\ dp\\ &= \psi (k) - \psi(N),
\end{align*}

donde $\psi$ es la función digamma.\\ %REVIEW : Referenciar artículo de Kozachenko Leonenko\\

Si asumimos que $f(x)$ es constante en la bola de radio $\epsilon$, tendremos \begin{align*}
p_i(\epsilon) &\approx \int_{B(x_i,\epsilon)}f(y)dy\approx f(x_i) \int_{B(x_i,\epsilon)}dy\\ &= f(x_i)\cdot V_d \cdot \epsilon^d.\\
\end{align*}
...
Continuamos tomando esperanzas respecto de $\varepsilon$ y sustituyendo la esperanza del logaritmo ya calculada, notamos que $\log f(X_i)$ no depende de $\epsilon$:\[
\log f(x_i) \approx \mathbb{E}\left[\log p_i(\epsilon)\right] - \log V_d  - d \mathbb{E} \left[\log \varepsilon\right] = \psi (k) - \psi (N) - \log V_d  - d \mathbb{E} \left[\log \varepsilon\right].
\]
...
El estimador anterior es el de Kozachenko - Leonenko en el caso $k=1$.\\

La forma de estimar $\widehat{\log f} (X_i)$ elimina una parte del sesgo, aunque se mantiene el sesgo proveniente de aproximar\[\int_{B(x_i,\epsilon)}f(y)dy\approx f(x_i) \int_{B(x_i,\epsilon)}dy.\] Si la densidad $f(x)$ es constante, el sesgo proveniente de esta aproximación también desaparece y obtenemos un estimador de la entropía, $\widehat{h}(X)$, insesgado.
...
Para la variable $Y$, $\varepsilon_k(z_i)$ no tendría por qué coincidir con la distancia entre $y_i$ y su $(n_y(i)+1)$-ésimo vecino. Sin embargo, consideramos la ecuación anterior una buena aproximación para $h(Y)$ (cambiando $X$ por $Y$). Esta aproximación será exacta cuando $n_y(i) \to \infty$ y cuando $N\to \infty$.---Referenciar---\\
...
\begin{align*}
  ...
  =& \ \psi(k) - \frac{1}{k} - \frac{1}{N}\sum_{i=1}^N\left( \psi(n_x(i)+1) + \psi(n_y(i)+1) \right ) + \psi(N).
\end{align*}
...
En este caso, $n_x(i)$ y $n_y(i)$ serán el número de puntos con $\Vert x_i - x_j \Vert \leq \varepsilon_k(x_i)$ y $\Vert y_i - y_j \Vert \leq \varepsilon_k(y_i)$ respectivamente. Tenemos que distinguir dos situaciones:

\begin{enumerate}[label={\alph*)}]
\item Los lados $\varepsilon_k(x_i)$ y $\varepsilon_k(y_i)$ quedan determinados por el mismo punto.
\item Los lados $\varepsilon_k(x_i)$ y $\varepsilon_k(y_i)$ quedan determinados por puntos diferentes.
\end{enumerate}
...
\]
donde $q_i \equiv q_i(\varepsilon_k(X), \varepsilon_k(Y))$ es la función de densidad del rectángulo de lados $\varepsilon_k(X) \times \varepsilon_k(Y)$ centrado en el punto $(X_i, Y_i)$, y $p_i$ es la función de densidad del cuadrado de lado $\varepsilon = \max \{\varepsilon_k(X), \varepsilon_k(Y)\}$. Esta última es necesaria porque usando la norma del máximo garantizamos que no hay puntos en ese cuadrado que no estén en el rectángulo.

Se verifica que $\int P_k =1$. Calculamos la esperanza de $\log q_i$:
...
Volviendo a aproximar $\log f(x_i)$ por \[
\log f(x_i) \approx  \log q_i(\varepsilon) - \log V_d - d \log \varepsilon,
\]
tomando esperanzas, obtenemos \[
\log f(x_i) \approx  \psi(k) - \frac{1}{k} - \psi(N) - \log V_d - d \E\log \varepsilon.
\]
Esto nos da el siguiente estimador de la entropía
 \begin{align*}
  \widehat{h}(X) &= - \frac{1}{N} \sum_{i=1}^N\widehat{\log f} (X_i) = - \frac{1}{N} \sum_{i=1}^N\left (\psi (k) - \frac{1}{k} - \psi (N) - \log V_d  - d\log \varepsilon \right ) \\
  &= - \psi (k) + \frac{1}{k}+ \psi (N) + \log V_d + \frac{d}{N} \sum_{i=1}^N \log \varepsilon (x_i). 
 \end{align*}

 Como ya hicimos en el primer algoritmo, utilizaremos el Lema \ref{l:ent_im} para estimar la información mutua, esta vez a partir del estimador de la entropía recién conseguido.

\begin{align*}
  I^{(2)}(X,Y) \approx& \ \widehat{h}(X) + \widehat{h}(Y) - \widehat{h}(X, Y)\\ =& - \frac{1}{N}\sum_{i=1}^N\psi(n_x(i)+1) + \frac{1}{n_x(i)+1} + \psi(N) + \log V_{d_X} + \frac{d_X}{N} \sum_{i=1}^N\log \varepsilon(i)\\
  & - \frac{1}{N}\sum_{i=1}^N\psi(n_Y(i)+1) + \frac{1}{n_y(i)+1} + \psi(N) + \log V_{d_Y} + \frac{d_Y}{N} \sum_{i=1}^N\log \varepsilon(i)\\
...
\texttt{cProfile} mide el número de veces que se ejecuta cada función, el tiempo total empleado en cada función, el tiempo empleado en cada función cada vez que se llamó a la función, el tiempo acumulado de cada función y todas a las que estas llamen y el tiempo acumulado entre el número de veces que se llame a esta función. Esta herramienta será precisa incluso en el caso de funciones recursivas, al distinguir el número de veces que se llama a la función del número de veces que fue invocada de forma recursiva.\\
...
Se ha realizado un análisis de los tiempos de ejecución de las dos implementaciones de la entropía y de las dos implementaciones de la información mutua vistas. Para ello, se han ejecutado 5 veces y nos hemos quedado con los tiempos medios de ejecución para diferentes valores del número de muestras y dimensión de las mismas. Remarcamos que el tiempo de ejecución considerado es el tiempo acumulado por la función a estudiar y todas las funciones a las que esta llame.\\

Se han considerado tres rangos para el tamaño de las muestras: pequeñas (1000 muestras), medianas (30000 muestras) y grandes (100000). De forma similar, en función a la dimensión, se realizaron estimaciones con tres tipos de dimensiones: pequeñas (dimensión 2), medianas (dimensión 10) y grandes (dimensión 25). Notamos que en el caso de la dimensión 25 debido a los elevados tiempos de ejecución, se repiten las mediciones 3 veces en vez de 5.\\

Comenzaremos analizando los tiempos obtenidos al calcular la entropía. En la Tabla \ref{tab:tiempos_ent_sep} vemos los tiempos de de ejecución de ambas implementaciones para los valores de $n$ y $d$ estudiados. Como es de esperar, a medida que aumenta el número de muestras y la dimensión, se incrementan los tiempos de ejecución en ambos casos.

\begin{table}[!htb]
  \begin{adjustwidth}{-.5in}{-.5in}  
    \caption{Tiempos de ejecución de la función \texttt{entropy} para diferentes valores de $n$ y $d$.}
    \label{tab:tiempos_ent_sep}
    \begin{subtable}{.5\linewidth}
      \centering
        \caption{Implementación 1.}
        \begin{tabular}{l|rrr}
          \toprule
          \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
          \midrule
2 &   0.0022\,s & 0.0589\,s & 0.2340\,s\\
10 &   0.0144\,s& 2.3644\,s & 13.1121\,s\\
25 & 0.0703\,s & 32.1099\,s & 633.221\,s\\ 
\bottomrule
        \end{tabular}
    \end{subtable}
    \begin{subtable}{.5\linewidth}
      \centering
        \caption{Implementación 2.}
        \begin{tabular}{l|rrr}
          \toprule
          \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
          \midrule
2 &    0.0017\,s &  0.0503\,s &  0.1838\,s\\
10 &  0.0068\,s &   0.8612\,s &    4.6004\,s\\
25 & 0.04\,s & 30.1558\,s &  878.081\,s\\
\bottomrule
        \end{tabular}
    \end{subtable}
    \end{adjustwidth}
...
Para realizar la comparación atenderemos a la Tabla \ref{tab:tiempos_ent}, que contiene la misma información estructurada de forma que resulte más fácil comparar. Cuando la dimensión es 2, los tiempos son similares para muestras pequeñas y medianas, diferenciándose más para muestras grandes. Sin embargo, en casi todos los casos la implementación 1 es más lenta que la 2. Al aumentar la dimensión, se incrementan los tiempos de ejecución de ambas implementaciones, pero también las diferencias entre ellas. Para una muestra pequeña la implementación 1 tarda aproximadamente el doble que la 2. Con muestras mayores la diferencia de tiempo aumenta, para una muestra grande, la implementación 1 tarda casi el triple que la 2. Destaca que para una dimensión grande y un tamaño de muestra grande se invierte la tendencia observada en el resto de combinaciones de tamaño de muestra y dimensión.\\

\begin{table}[H]
\centering
\caption{Tiempos de ejecución de la función \texttt{entropy} en ambas implementaciones para diferentes valores del parámetro $d$ y $n$.}
\label{tab:tiempos_ent}
\begin{tabular}{r|l|rrr}
  \toprule
Implementación & \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
 \midrule
1 & 2 &   0.0022\,s & 0.0589\,s & 0.2340\,s\\
2 & 2  &    0.0017\,s &  0.0503\,s &  0.1838\,s\\[5pt]
1 & 10 &   0.0144\,s& 2.3644\,s & 13.1121\,s\\
2 & 10 &  0.0068\,s &   0.8612\,s &    4.6004\,s\\[5pt]
1 & 25 & 0.0703\,s & 32.1099\,s & 633.221\,s\\
2 & 25 & 0.04\,s & 30.1558\,s &  878.081\,s\\
\bottomrule
\end{tabular}
\end{table}
...
Si analizamos qué pasos emplean más tiempo en las implementaciones de la entropía nos sorprende descubrir que es en la misma función, el método \texttt{query} de \texttt{sklearn.neighbors.\_kd\_tree.BinaryTree}. Sin embargo, aunque ambas implementaciones llaman a este método, la primera pasa más tiempo en ella que la segunda. Por ejemplo, para $d=10$ y $n = 100000$, la implementación 1 empleó $17.075\,$s (de una ejecución de $17.2516\,$s) en la función \texttt{query}, mientras que la implementación 2 usó $6.057\,$s en esta función (de una ejecución que en total duró $6.2308\,$s). \\

En la implementación 1 se obtienen los vecinos más cercanos utilizando la clase \texttt{sklearn.neighbors.NearestNeighbors}, llamando al método \texttt{kneighbors}. Este método realiza una serie de comprobaciones antes de llamar al método \texttt{query} para obtener realmente los vecinos más cercanos y sus distancias. Además, la llamada a \texttt{query} se realiza usando métodos que proporcionan paralelismo de la biblioteca \texttt{joblib}, indicando que se utilizará paralelismo a nivel de hebra.\\

En la implementación 2, se crea un árbol de tipo \texttt{sklearn.neighbors.BallTree} y luego se calculan los vecinos más cercanos llamando directamente al método \texttt{query}.\\

Suponemos que el coste de realizar la llamada en paralelo sin aprovechar realmente el paralelismo es el que provoca el incremento en las llamadas a \texttt{query} en la primera implementación y que por eso en la segunda implementación, en la que se llama al método directamente, es más rápida. No se llega a concluir nada acerca de la anomalía para tamaños de muestra grandes y dimensión grande.\\
...
En la Tabla \ref{tab:tiempos_im_sep} se recogen los tiempos de ejecución de las dos implementaciones para los valores de $n$ y $d$ considerados.
Al igual que para la entropía, los tiempos en todos los casos aumentan al considerar muestras más grandes y de mayor dimensión. Además, notamos que los tiempos de ejecución son superiores a los necesitados para calcular la entropía, esto no es de extrañar, en uno de los casos se calcula la entropía de varias variables y luego se opera con ellas para obtener la información mutua.\\
\begin{table}[!htb]
  \begin{adjustwidth}{-.5in}{-.5in}  
    \caption{Tiempos de ejecución del cálculo de la información mutua para diferentes valores de $n$ y $d$.}
    \label{tab:tiempos_im_sep}
    \begin{subtable}{.5\linewidth}
      \centering
        \caption{Implementación 1.}
        \begin{tabular}{l|rrr}
          ...
          \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
          \midrule
2 &   0.0066\,s &  0.2852\,s &  1.1668\,s\\
10 &   0.091\,s &   28.3134\,s &  380.604\,s\\
25 & 0.2218\,s & 177.259\,s & 3194.09\,s\\
\bottomrule
\end{tabular}
\end{subtable}
    \begin{subtable}{.5\linewidth}
      \centering
        ...
        \begin{tabular}{l|rrr}
          ...
          \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
          \midrule
2  &    0.0065\,s &  0.4432\,s &  2.2371\,s \\
10 &  0.0504\,s &  32.9856\,s &  783.608\,s\\
25 & 0.1213\,s & 153.015\,s & 3044.07\,s\\
\bottomrule
        \end{tabular}
    \end{subtable}
    \end{adjustwidth}
...
Compararemos los tiempos de ejecución de ambas implementaciones utilizando la Tabla \ref{tab:tiempos_mi}. Para un tamaño de muestra pequeña y dimensión pequeña, los tiempos son muy similares, pero a medida que van aumentando, tanto dimensión como tamaño de muestra, aumenta también la diferencia entre los tiempos de ejecución de las dos implementaciones. En este caso ocurre al contrario que al calcular la entropía, la implementación que tarda más en estimar la información mutua es, en todos los casos menos para una muestra grande y dimensión grande, la segunda, tardando el doble para muestras grandes con dimensión 10 y más del cuádruple para muestras medianas y dimensión grande.\\
...
Implementación & \backslashbox{$d$}{$n$} & 1000  & 30000 & 100000\\
 \midrule
1 & 2 &   0.0066\,s &  0.2852\,s &  1.1668\,s\\
2 & 2  &    0.0065\,s &  0.4432\,s &  2.2371\,s \\[5pt]
1 & 10 &   0.091\,s &   28.3134\,s &  380.604\,s\\
2 & 10 &  0.0504\,s &  32.9856\,s &  783.608\,s\\[5pt]
1 & 25 & 0.2218\,s & 177.259\,s & 3194.09\,s\\
2 & 25 & 0.1213\,s & 153.015\,s & 3044.07\,s\\
\bottomrule
\end{tabular}
\end{table}

En la primera implementación, es el método \texttt{query} el que emplea más tiempo de ejecución (69.5\,s de una ejecución de 70.2\,s para dimensión 10), se llama a este método 3 veces, una para cada entropía a estimar, las marginales y la conjunta. En la segunda implementación, además de usar el método \texttt{query} (en este caso una sola vez) para calcular los vecinos más cercanos se utiliza el método \texttt{query\_radius} para calcular el valor de la media de $\psi$ (es necesario calcular 2 valores diferentes). Para una ejecución con $n=100000$ y $d=10$ que tardó $425.95\,$s, se emplearon $414.94\,s$ en la función \texttt{query} y $10.45\,$s en \texttt{query\_radius}, el cuello de botella es, por tanto, la función \texttt{query}.---¿Por qué tarda mucho más para la segunda implementación que para la primera?---
