...
Cuando $n=1$ diremos que la variable aleatoria unidimensional, mientras que cuando $n >1$ diremo que la variable aleatoria es multidimensional, en ocasiones también denominada, vector aleatorio.---¿Se te ocurre algo más que poner aquí?--- En el caso de la variable aleatoria multidimensional podremos querer información sobre ella misma, $X=(X_1,\cdots, X_n)$, lo que llamaramos variable aleatoria conjunta, o sobre alguna de las variables aleatoria $X_i$, que denominaremos variable aleatoria marginal.---Esto me suena raro...lo dejarías ahí o en otro lado...¿quizás donde lo de la función de distribución?---\\ %REVIEW

Cuando la imagen de la variable aleatoria $X$ sea numerable, diremos que la variable aleatoria es \textit{discreta}, será descrita por la función masa de probabilidad que asigna una probabilidad a cada valor de la imagen de $X$. Por otro lado, cuando la imagen de la variable aleatoria $X$ sea infinita no numerable, diremos que la variable aleatoria es \textit{continua}. Si además pede ser descrita por una función de densidad, será \textit{absolutamente continua}.\\
...
\begin{proposition}
  ...
  ---Diciendo esto...¿tiene interés decir lo de que es continua a la derecha en general?---
\end{proposition}
...
??? = \frac{???}{???}. %REVIEW
  \]---Se me ocurre que puede ser $f_X(x|Y=y) = \frac{f(x,y)}{f_Y(y)}$---
\end{definition}
...
% REVIEW: Añadir ejemplos de estimador de la media y la varianza
...
Medir la dependencia entre variables puede ser útil porque nos indica relaciones entre variables que se pueden aprovechar para hacer predecciones de una a partir de la otra. Encontramos infinidad de aplicaciones en el mundo real a conocer una dependencia entre variables. Conocer qué efecto tiene un fenómeno sobre otro...---¿Ejemplos? ¿ideas para rellenar esto?---
...
\]%REVIEW
a) El mismo coeficiente para una muestra $\{(x_1,y_1),\dots,(x_n,y_n)\}$ se define como
...
h(X) = - \int_0^a\log\frac{1}{a}\frac{1}{a}dx = \log a.
  \]
...
\begin{example}\label{ej:norm_uni}
  ...
  \begin{align*}
    h(X) &= - \int_{\mathbb{R}} \log f(x) f(x)  dx = - \int_{\mathbb{R}}  \log \left ( \frac{1}{\sigma \sqrt{2\pi}} e ^{-\frac{(x-\mu)^2}{2\sigma ^2}} \right ) \frac{1}{\sigma \sqrt{2\pi}} e ^{-\frac{(x-\mu)^2}{2\sigma ^2}}dx\\
...
D(f||g) = \int_{\mathbb{R}^n} \log \frac{f(x)}{g(x)}f(x) dx.
  \]
\end{definition}
...
I(X;Y) = \int \int \log \frac{f(x,y)}{f(x)f(y)}f(x,y) dxdy = D(f(x,y)||f(x)f(y)).
  \]
\end{definition}
...
\begin{proof}
  ...
  \begin{align*}
    I(X;Y) &= \int \int  \log \frac{f(x,y)}{f(x)f(y)}f(x,y)dxdy \\
    &= \int \int  \log f(x,y)f(x,y)dxdy - \int \int  \log f(x)f(y)dxdy\\
    &= -h(X,Y)-\int\int f(x,y) \log f(x) dy dx - \int \int f(x,y)\log f(y)f(x,y)dxdy\\
...
\begin{proof}
  ...
  \begin{align*}
    I(X;Y)&= \int \int  \log \frac{f(x,y)}{f(x)f(y)}f(x,y)dxdy \\
    &= - \int\int \log f(x)f(x,y)  dx dy + \int\int\log \frac{f(x,y)}{f(y)}  f(x,y)dx dy\\
...
\begin{theorem}[Regla de la cadena para la entropía diferencial]
  \[
...
\begin{proof}
  Sea $f_X$ la función de densidad de $X$. Llamemos $Y = X+ c$, esta será una variable aleatoria con función de densidad $f_Y(y)=f_X(y-c)$. Calculamos su entropía diferencial usando su definición y aplicando el cambio de variable $y = x + c$.
  \begin{align*}
    h(X+c) &= h(Y) = - \int \log f_Y(y)f_Y(y) dy\\
    &= - \int  \log f_X(y-c) f_X(y-c) dy\\
    &= - \int  \log f_X(x) f_X(x) dx = h(X).
  \end{align*}
\end{proof}

\begin{theorem}
  Sea $X$ una variable aleatoria unidimensional y $a$ un escalar, entonces\[
h(aX) = h(X) + \log |a|.
  \]
\end{theorem}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= -\int \log \left (f_X\left (x \right ) \right ) \frac{a}{|a|}f_X\left (x \right ) dx + \int\log{|a|} \frac{a}{|a|}f_X(x)dx\\%REVIEW
    &= h(X) + \log{|a|}.
  \end{align*}
  ---¿Por qué toma a/|a| = 1? ¿No podría ser -1? ---
\end{proof}
