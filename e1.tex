...
\begin{example}\label{ej:norm_uni}
  Consideramos una variable aleatoria $X$ con ditribución normal univariante de media $\mu$ y varianza $\sigma$, su función de densidad es $f(x) = \frac{1}{\sigma \sqrt{2\pi}} e ^{-\frac{(x-\mu)^2}{2\sigma ^2}}$, queremos calcular su entropía.
  \begin{align*}
    ...
    &= -  \frac{1}{\sigma \sqrt{2\pi}} \int e ^{-\frac{(x-\mu)^2}{2\sigma ^2}} \left [ \log \left ( e ^{-\frac{(x-\mu)^2}{2\sigma ^2}} \right ) - \log \sigma \sqrt{2\pi} \right ] dx\\
    &= -  \frac{1}{\sigma \sqrt{2\pi}}\left [ \int e ^{-\frac{(x-\mu)^2}{2\sigma ^2}} \left ( -\frac{(x-\mu)^2}{2\sigma ^2} \right )dx - \int e ^{-\frac{(x-\mu)^2}{2\sigma ^2}} \log \sigma \sqrt{2\pi} dx \right ].\\
  \end{align*}

  Ahora, utilizando la ley del estadístico incosciente ($E[g(X)]=  \int g(x)f(x)$), tenemos,
  \begin{align*}
    h(X) &= \frac{\mathbb{E}\left [ (X-\mu) ^2\right ] }{2\sigma^2} + \log \sigma\sqrt{2\pi} \underbrace{\int \frac{1}{\sigma\sqrt{2\pi}}e ^{-\frac{(x-\mu)^2}{2\sigma ^2}}dx}_{1}\\
    &= \frac{\sigma^2}{2\sigma^2} + \log \sqrt{\sigma^2 2 \pi} = \frac{1}{2} + \frac{1}{2}\log  \sigma^2 2 \pi\\
    &= \frac{1}{2} \log e + \frac{1}{2} \log \sigma^2 2 \pi\\ &= \frac{1}{2}\log{2\pi e\sigma^2} \text{ nats,}
  \end{align*}

donde se ha utilizado que $\mathbb{E}[(X-\mu)^p] = \sigma^p(p-1))!!$ si $p$ es par.
...
\begin{definition}[Entropía diferencial]
  La entropía diferencial de un conjunto de variables aleatorias $X_1,\cdots,X_n$ con función de densidad $f(x_1,\cdots,x_n)$ se define como\[
h(X_1,\cdots,X_n) = - \int f(x^n)\log f(x^n) dx^n.
  \]
\end{definition}
...
\begin{example}\label{ej:norm_multi}
  ...
  \begin{align*}
    ...
    &= - \int f(x) \left (- \frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu) - \log\left (\sqrt{2\pi} \right )^n |\Sigma|^{\frac{1}{2}}  \right )dx\\
    &= \frac{1}{2} \mathbb{E}\left [ (X-\mu)^T\Sigma^{-1}(x-\mu) \right ] + \frac{1}{2}\log(2\pi)^n|\Sigma|.
  \end{align*}
  ...
  \begin{align*}
    h(X) &= \frac{n}{2} + \frac{1}{2}\log(2\pi)^n|\Sigma| = \frac{1}{2} \left (\log e^n + \log (2\pi)^n|\Sigma| \right)\\
    &= \frac{1}{2}\log(2\pi e)^n|\Sigma| \text{ nats.}
  \end{align*}
...
Coincidiendo con la fórmula vista en \ref{ej:norm_multi} salvo porquese calcula el valor absoluto del determinante.
...
Observamos los errores obtenidos en la Tabla \ref{tab:err_ent}, vemos como a medida que aumenta la dimensión, aumenta el error de forma similar para ambos estimadores.
