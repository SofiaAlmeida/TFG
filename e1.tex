...
\begin{example}
La familia de conjuntos Borel $\mathcal{A} = \mathcal{B}(\mathbb{R})$ es una $\sigma$-álgebra sobre $\mathbb{R}$. Un conjunto Borel sobre un espacio topológico es aquel formado por todos los conjuntos abiertos con las operaciones de unión numerable, intersección numerable y complementario relativo.
\end{example}
...
% REVIEW: ¿Definir espacio de probabilidad?
...
\begin{definition}[Vector aleatorio]
    Sea $(\Omega, \mathcal{A}, P)$ un espacio de probabilidad. Un vector aleatorio $X=(X_1,\cdots, X_n)$ es una función medible \[
...
\begin{definition}[Probabilidad inducida]
  La probabilidad inducida por un vector aleatorio $X=(X_1,\cdots, X_n)$, o distribución de $X$, se define como la función \[
P_X[B]:= P[X^{-1}(B)],
  \]
...
\]
se dice que $X$ es un vector aleatorio con distribución absolutamente continua y $f_X$ se llama función de densidad de $X$. Si hay una secuencia (finita o infinita) de vectores $x_1,x_2,\cdots$ tal que para cualquier conjunto Borel $B\subset \mathbb{R}^n$\[
...
\begin{definition}[Vectores aleatorios independientes]
  Dos vectores aleatorios $X$ e $Y$ son independientes si para cualesquiera conjuntos de Borel $A, B \in \mathcal{B}(\mathbb{R}^n)$ los eventos $[X\in A]$ y $Y\in B$ son independientes. De forma general $k$ vectores aleatorios $X_1,\cdots,X_k$ serán independientes si para cualesquiera conjuntos de borel $B_1,\cdots,B_k \in \mathcal{B}(\mathbb{R}^n)$ los eventos $[X_1 \in B_1],\cdots,[X_k\in B_k]$ son independientes.
  
\end{definition}

\begin{proposition}[Caracterizaciones de independencia]
  Dado un vector aleatorio $X =(X_1,\cdots,X_n)$ sus componentes son independientes si, y solo si, se da alguna de las siguientes condiciones:
  \begin{itemize}
...
% REVIEW: DEFINIR ESPERANZA

\begin{definition}[Vector de medias]
  Sea $X=(X_1,\cdots,X_N)^T$ un vector aleatorio. Se define el vector de medias de $X$ como:\[
...
\begin{definition}[Matriz de convarianzas]
  Se define la matriz de covarianzas del vector $X= (X_1,\cdots,X_n)^T$ como\[
\Sigma = Cov(X) = \mathbb{E}\left [(X-\mu)(X-\mu)^T \right ] =  \begin{bmatrix} \sigma_{11} & \dots & \sigma_{1n} \\ \vdots& \ddots & \vdots \\ \sigma_{n1} &  \dots & \sigma_{nn}\end{bmatrix}\,,
\]
donde $\sigma_{ij} = \mathbb{E}\left[(X_i-\mu_i)(X_j-\mu_j)\right]=\sigma_{ji}$ es la covarianza de las variables aleatorias $X_i$ y $X_j$. La podremos definir cuando existan todas las covarianzas.  
\end{definition}
...
La distribución multinomial modela la probabilidad de un número determinado de éxitos para $n$ sucesos independientes. Donde hay $k$ posibles éxitos, cada uno de ellos con probabilidad de éxito fija.

\begin{definition}[Distribución multinomial]
  Sea $k$ un número fijo y finito, tendremos $k$ posibles salidas mutuamente excluyentes con propabilidades $p_1\cdots p_k$. Si la variable aleatoria $X_i$ indica el número de veces que se observa la salida $i$ en los $n$ sucesos, entonces el vector $X = (X_1, \cdots, X_n)$ sigue una distribución multinomial con parámetros $n$ y $p = (p_1,\cdots, p_n)$. Su función masa de probabilidad viene dada por
