...
Medir la dependencia entre variables puede ser útil porque nos indica relaciones predecibles entre las variables que pueden ser aprovechadas en la práctica. Encontramos infinidad de aplicaciones en el mundo real a conocer una dependencia entre variables. Conocer qué efecto tiene un fenómeno sobre otro...---¿Ejemplos?---
...
Entre los no paramétricos (que no dependen de las distribuciones marginales)
...
\begin{definition}[Entropía diferencial]
  La entropía diferencial $h(X)$ de una variable aleatoria continua $X$ \[
...
-\frac{1}{n}\log f(X_1,\cdots, X_n) \to \mathbb{E}\left [ - \log f(X) \right ] = h(X) \text{ en probabilidad.}
  \]
\end{theorem}
...
D(f||g) = \int_{\mathbb{R}^n} f(x) \log \frac{f(x)}{g(x)}.
  \]
\end{definition}

Notamos que $D(f||g)$ es finita si, y solo si, el conjunto soporte de $f$ está contenido en el conjunto soporte de $g$. ---Esto no sé por qué---
Al igual que en el caso discreto y motivados por la continuidad utilizaremos $0\log\frac{0}{0} = 0$.

\begin{definition}[Información mutua]
  La información mutua, $I(X;Y)$ entre dos variables aleatorias con función de densidad conjunta $f(x,y)$ viene dada por\[
I(X;Y) = \int \int f(x,y) \log \frac{f(x,y)}{f(x)f(y)}dxdy = D(f(x,y)||f(x)f(y)).
  \]
\end{definition}
...
\begin{proof}
  Supongamos que las funciones de densidad marginales son $f(x)$, $f(y)$ y la conjunta es $f(x,y)$.
...
\begin{proof}
  ...
  \begin{align*}
    I(X;Y)&= \int \int f(x,y) \log \frac{f(x,y)}{f(x)f(y)}dxdy \\
    &= - \int\int f(x,y) \log f(x) dx dy + \int\int f(x,y)\log \frac{f(x,y)}{f(y)} dx dy\\
    &= - \int f(x) \log f(x) dx + \int\int f(x,y)\log f(x|y) dx dy\\  
    &= h(X) - h(X|Y).
  \end{align*}
...
\begin{proof}
  ...
  \begin{align*}
    h(X+c) &= h(Y) = - \int f_Y(y)\log f_Y(y) dy\\
    &= - \int f_X(y-c) \log f_X(y-c) dy\\
    &= - \int f_X(x) \log f_X(x) dx = h(X).
  \end{align*}
\end{proof}

\begin{theorem}
  Sea $X$ una variable aleatoria absolutamente continua y $a$ un escalar, entonces\[
h(aX) = h(X) + \log |a|.
  \]
\end{theorem}
...
\begin{proof}
  ...
  \begin{align*}
    h(aX) &= h(Y) = -\int f_Y(y)\log \left(f_Y(y)\right) dy\\
    &=  -\int \frac{1}{|a|}f_X\left (\frac{y}{a} \right ) \log \left ( \frac{1}{|a|}f_X\left (\frac{y}{a} \right ) \right ) dy\\
    &= -\int \frac{a}{|a|}f_X\left (x \right ) \log \left ( \frac{1}{|a|}f_X\left (x \right ) \right ) dx\\
    &= -\int \frac{a}{|a|}f_X\left (x \right ) \log \left (f_X\left (x \right ) \right ) dx + \int \frac{a}{|a|}f_X(x)\log{|a|}dx\\%REVIEW
    &= h(X) + \log{|a|}.
  \end{align*}
\end{proof}

\begin{corollary}
  Si $X$ es una variable aleatoria y $A$ una matriz entonces,\[
h(AX) = h(X) + \log|det(A)|.
  \]
\end{corollary}
